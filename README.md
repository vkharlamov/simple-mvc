...
Простейший MVC-подход исключительно в целях демонстрации.
Улучшать архитектуру можно вплоть до копирования Laravel или Yii2 - не ставил перед собой такой цели.

Глобальное хранилище объектов организовано через Service Locator.
Отсутствует слой ORM между Моделью и БД, потому фабрика подключения драйвера БД имеет номинальный\демонстрационный
смысл, т.е. в моделях прямые запросы.

- Нет автоматического внедрения зависимостей ни через конструктор, ни через сеттеры.
- Используется предельно простой обработчик ошибок.
- Вместо сортировки по полю "статус" (условие в ТЗ) используется фильтр.
- Простой но быстрый алгоритм хеширования(пароль,метод проверки текста на существова ).
- Вынести session_start(); в метод проверки авторизации пользоватля. Конкретно в этом случае незачем создавать сессию для "гостя"  см. https://github.com/vkharlamov/simple-mvc/blob/fd7577c85c7f98be042d589cd2bc1c614b2dd6f9/public/index.php#L15

P.S.
Конечно, можно было подключить Symfony-компоненты Response, Request, маршрутизатор итд.

------------------------------------------------------------------

Пример https://tinymvc.000webhostapp.com/
- user: admin
- pass: 123

------------------------------------------------------------------

Привожу оригинальный текст постановки задачи:
